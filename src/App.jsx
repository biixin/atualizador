...
async function api(path, options = {}) {
  // Authentication should recover promptly; SCORA checks and push sends may take longer.
  const timeout = /^\/(auth|login|admin\/(session|login|logout))$/.test(path) ? 15_000 : 55_000;
  let response;
  try {
    response = await fetch(`/api${path}`, { ...options, headers: { 'Content-Type': 'application/json', ...options.headers }, signal: AbortSignal.timeout(timeout) });
  } catch (error) {
    if (error.name === 'TimeoutError' || error.name === 'AbortError') throw new Error('O servidor demorou para responder. Tente novamente em alguns instantes.');
    throw new Error('Não foi possível conectar ao servidor. Confira sua conexão e tente novamente.');
  }
  if (!response.headers.get('content-type')?.includes('application/json')) {
    throw new Error(response.status === 404 ? 'A API do radar não foi publicada. Atualize o projeto completo, incluindo as pastas api e server e o arquivo vercel.json, e faça um novo deploy.' : 'A hospedagem retornou uma página em vez dos dados da API. Confira a configuração do deploy.');
  }
...
function Toggle({ value, onChange, label, disabled }) { return <button type="button" className={`toggle ${value ? 'on' : ''}`} role="switch" aria-label={label} aria-checked={value} aria-busy={!!disabled} onClick={() => onChange(!value)} disabled={disabled}><span /></button>; }
function Modal({ title, children, onClose, toast, onDismiss, setupNotice }) {
  const dialog = useRef(null);
  useEffect(() => { dialog.current.showModal(); }, []);
  return <dialog ref={dialog} className="modal" onCancel={onClose} onClick={event => { if (event.target === dialog.current) onClose(); }}><div className="modal-head"><h2>{title}</h2><button className="icon-button" aria-label="Fechar" onClick={onClose}><X size={20} /></button></div>{setupNotice && <div className="modal-toast"><span>Antes de ativar os avisos, conclua a configuração da hospedagem.</span><button onClick={setupNotice}>Configurar</button></div>}{children}{toast && <div className={`modal-toast ${toast.error ? 'error' : ''}`} role={toast.error ? 'alert' : 'status'}><span>{toast.message}</span><button aria-label="Fechar aviso" onClick={onDismiss}><X size={15} /></button></div>}</dialog>;
}
function EventList({ events, now, expanded = false, persistent = true }) {
  if (!persistent) return <div className="empty-history"><History size={26} /><p>Histórico ainda não configurado.</p><small>Conecte o armazenamento da hospedagem para registrar as mudanças.</small></div>;
...
export default function App() {
  ...
  useEffect(() => {
    api('/auth').then(setAuth).catch(error => setOffline(error.message || 'Não foi possível conectar ao servidor.'));
  ...
  useEffect(() => {
    ...
    const update = async () => {
      ...
      catch (error) { if (active) { if (error.status === 401) { setAuth({ authenticated: false, required: true }); setData(null); } else setOffline(error.message || 'Conexão com o servidor interrompida. Os dados abaixo podem estar desatualizados.'); } }
  ...
  async function settings(patch) {
    if (saving) return;
    if (!data) return notify('Aguarde os dados do radar carregarem. Se houver um erro de conexão, clique em Tentar novamente.', true);
    if (data.capabilities?.preferences === false) { setModal('hosting'); return; }
    setSaving(true);
  ...
  async function enablePush() {
    if (data?.capabilities?.push === false) { setModal('hosting'); return; }
  ...
  const lastChangeLabel = capabilities?.persistent === false ? 'Histórico ainda não configurado' : relative(data?.lastChange, now);
  const vercel = capabilities?.runtime === 'vercel';
  const intervals = vercel ? INTERVALS.filter(item => item.value >= 60) : INTERVALS;
    ...
    {auth && !auth.authenticated && page !== 'admin' ? <main className="login-screen"><div className="login-card"><Brand /><h1>Seu radar, só seu.</h1><p>Digite a senha de acesso definida para este painel.</p><form onSubmit={login}><label>Senha do painel<input autoFocus type="password" autoComplete="current-password" value={password} onChange={event => setPassword(event.target.value)} required /></label><button className="button primary" disabled={busy} aria-busy={busy}>{busy ? <LoaderCircle className="spin" size={17} /> : <ShieldCheck size={17} />} Entrar no painel</button></form><button type="button" className="text-button" onClick={() => setPage('admin')}>Entrar como administrador</button><small>Esta é a senha do RAS Radar, não a sua senha do SCORA.</small></div></main> : <div className="app-shell">
      ...
      <div className="main-wrap">
        ...
        <main className="content">
          ...
          {problem && <div className="error-banner" role="alert"><WifiOff size={19} /><div><strong>Não foi possível atualizar o radar</strong><p>{problem}</p></div><button onClick={auth ? check : () => window.location.reload()} disabled={busy} aria-busy={busy}>Tentar novamente</button></div>}
          ...
          {page === 'dashboard' && <>
            ...
            <div className="dashboard-grid"><div className="opportunities card"><div className="card-heading"><div><h2>Vagas de RAS <span className="count-badge">{eligible.length}</span></h2><p>Encontre a próxima data para a sua escala.</p></div><button className="button compact" onClick={check} disabled={busy || !auth?.authenticated} aria-busy={busy}><RefreshCw size={15} className={busy || data?.checking ? 'spin' : ''} />Verificar agora</button></div><div className="vacancy-tabs" role="tablist" aria-label="Status das vagas">{[{ id: 'all', label: 'Todos os RAS' }, { id: 'open', label: 'Abertos' }, { id: 'full', label: 'Esgotados' }, { id: 'closed', label: 'Encerrados' }].map(item => <button key={item.id} role="tab" aria-selected={tab === item.id} className={tab === item.id ? 'selected' : ''} onClick={() => setTab(item.id)}>{item.label}{item.id === 'open' && <span>{open.length}</span>}</button>)}</div><div className="list-tools"><label className="search-input"><Search size={17} /><input aria-label="Buscar por local" placeholder="Buscar por local…" value={search} onChange={event => setSearch(event.target.value)} /></label><button className="sort-button" onClick={() => setSort(sort === 'asc' ? 'desc' : 'asc')} aria-label="Alterar ordem das datas"><ArrowDownUp size={15} /><span>{sort === 'asc' ? 'Mais próximas' : 'Mais distantes'}</span></button></div><div className="date-filters"><button className={date === 'all' ? 'selected' : ''} onClick={() => setDate('all')}><CalendarDays size={16} /><span>Todas as datas</span></button>{dates.slice(0, 8).map(day => <button key={day} className={date === day ? 'selected' : ''} onClick={() => setDate(day)}>{formatDate(day)} <span className="day-week">{weekday(day)}</span></button>)}{dates.length > 8 && <select aria-label="Outras datas" value={date} onChange={event => setDate(event.target.value)}><option value="all">Outras datas</option>{dates.map(day => <option key={day} value={day}>{formatDate(day)}</option>)}</select>}</div>
              ...
              <aside className="right-column"><section className="card monitor-config"><div className="section-title"><span className="soft-icon"><Settings2 size={18} /></span><h2>Seu monitor</h2></div><label className="field-label" htmlFor="interval">Verificar a cada</label><select id="interval" value={config.interval} onChange={event => settings({ interval: Number(event.target.value) })} disabled={saving} aria-busy={saving}>{intervals.map(item => <option key={item.value} value={item.value}>{item.label}</option>)}</select><div className="notification-row"><span><strong>Avisos neste aparelho</strong><small>{pushEnabled ? 'Notificações ativadas' : 'Receba quando uma vaga abrir'}</small></span><Toggle label="Configurar avisos neste aparelho" value={pushEnabled} onChange={() => setModal('push')} /></div><div className="config-separator" /><div className="connection-line"><span><i className={healthy ? 'green-dot' : 'gray-dot'} />{healthy ? 'SCORA conectado' : data?.error ? 'SCORA indisponível' : 'Aguardando SCORA'}</span><ExternalLink size={12} /></div><div className="check-line"><span>Última consulta</span><strong>{formatTime(data?.lastCheck)}</strong></div><div className="check-line"><span>Consultas realizadas</span><strong>{data?.checks || 0}</strong></div><button className="text-button" onClick={() => setPage('settings')}>Ajustar preferências <ArrowRight size={14} /></button></section>
              <section className="card recent-activity"><div className="section-title"><h2>Últimas atualizações</h2><span className="subtle-icon"><History size={17} /></span></div><EventList persistent={capabilities?.persistent !== false} events={(data?.events || []).slice(0, 3)} now={now} /><button className="history-link" onClick={() => setPage('history')}>Ver histórico completo <ArrowRight size={14} /></button></section>
          ...
          {page === 'history' && <section className="card history-page"><div className="card-heading"><div><h2>O que mudou no SCORA</h2><p>Até 200 registros mais recentes. Horários de detecção pelo radar.</p></div><span className="count-badge">{data?.events.length || 0}</span></div><EventList persistent={capabilities?.persistent !== false} events={data?.events || []} now={now} expanded /></section>}
          {page === 'settings' && <div className="settings-grid"><section className="card settings-card"><span className="soft-icon"><Radar size={22} /></span><h2>Monitoramento</h2><p>As preferências valem para o monitor e todos os seus dispositivos.</p><div className="setting-row"><div><strong>Monitoramento automático</strong><small>Continuar verificando mesmo com o painel fechado.</small></div><Toggle label="Monitoramento automático" value={config.enabled} onChange={enabled => settings({ enabled })} disabled={saving} aria-busy={saving} /></div><label>Intervalo de verificação<select value={config.interval} onChange={event => settings({ interval: Number(event.target.value) })} disabled={saving} aria-busy={saving}>{intervals.map(item => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label><label>Sua escala de trabalho<select value={config.workdays} onChange={event => { setDate('all'); settings({ workdays: event.target.value }); }} disabled={saving} aria-busy={saving}><option value="all">Acompanhar todos os dias</option><option value="even">Trabalho nos dias pares → RAS nos ímpares</option><option value="odd">Trabalho nos dias ímpares → RAS nos pares</option></select></label><p className="field-hint">O filtro seleciona seus dias de folga para a lista e os avisos. Sua escala oficial continua sendo a cadastrada no SCORA.</p><div className="setting-row"><div><strong>Avisar também sobre outras alterações</strong><small>Incluir mudanças de horário, encerramentos e remoções.</small></div><Toggle label="Avisar sobre outras alterações" value={config.notifyChanges} onChange={notifyChanges => settings({ notifyChanges })} disabled={saving} aria-busy={saving} /></div></section><section className="card settings-card"><span className="soft-icon"><Smartphone size={22} /></span><h2>Alertas no celular</h2><p>O servidor acompanha as vagas e envia as notificações para os dispositivos conectados.</p><div className="device-status"><span className={pushEnabled ? 'green-dot' : 'gray-dot'} /><div><strong>{pushEnabled ? 'Este aparelho está conectado' : 'Este aparelho ainda não recebe alertas'}</strong><small>{data?.devices || 0} dispositivo(s) cadastrado(s)</small></div></div><button className="button primary" onClick={() => setModal('push')}><Bell size={16} />Configurar notificações</button>{data?.pushError && <p className="field-error">{data.pushError}</p>}<div className="info-note"><CircleHelp size={18} /><p>Para receber com o painel fechado, o servidor precisa continuar ligado. No celular, acesse pelo endereço HTTPS do seu site.</p></div><a className="text-button" href="https://developer.mozilla.org/pt-BR/docs/Web/API/Push_API" target="_blank" rel="noreferrer">Sobre as notificações push <ArrowUpRight size={14} /></a></section></div>}
