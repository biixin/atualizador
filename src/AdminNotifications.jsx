...
export default function AdminNotifications({ api, onSessionChange, onSetupHosting }) {
  ...
  async function send(event) {
    event.preventDefault();
    if (busy || cooldown > 0 || explainUnavailable()) return;
    setBusy(true); setFailure(''); setReport(null);
    try {
  ...
  function explainUnavailable() {
    if (!overview) { setFailure('A lista de aparelhos ainda não carregou. Clique em Atualizar dispositivos e tente novamente.'); return true; }
    if (!overview.available) { onSetupHosting(); return true; }
    if (!recipients.length) { setFailure('Nenhum aparelho nesta seleção. Abra o site no celular, ative os alertas e clique em Atualizar dispositivos.'); return true; }
    return false;
  }
  ...
  if (!session?.authenticated) return <section className="card admin-login">
    <div className="admin-lock"><LockKeyhole size={26} /></div>
    <div className="eyebrow">ACESSO RESTRITO</div>
    <h2>Entre como administrador</h2>
    <p>Use seu login e senha de admin para enviar mensagens de teste aos celulares cadastrados.</p>
    {session?.configured === false && <div className="admin-setup-note">
      <strong>O acesso de admin precisa ser configurado no servidor.</strong>
      <p>Defina <code>ADMIN_USERNAME</code> e <code>ADMIN_PASSWORD</code> com uma senha de pelo menos 12 caracteres. No computador, altere o arquivo <code>.env</code> e reinicie o servidor. Na Vercel, salve as variáveis em Settings → Environment Variables e faça um novo deploy.</p>
    </div>}
    {errorNotice}
    <form onSubmit={login}>
      <label>Login de administrador<input autoComplete="username" name="username" value={username} onChange={event => setUsername(event.target.value)} maxLength={80} required /></label>
      <label>Senha de administrador<input type="password" autoComplete="current-password" name="password" value={password} onChange={event => setPassword(event.target.value)} maxLength={512} required placeholder="Digite sua senha" /></label>
      <button type="submit" className="button primary full-width" disabled={busy} aria-busy={busy}>
        {busy ? <LoaderCircle size={17} className="spin" /> : <ShieldCheck size={17} />}
        {busy ? 'Entrando…' : 'Entrar como administrador'}
      </button>
    </form>
    <small>O acesso administrativo permite testar os avisos pelo computador. Você não precisa ativar notificações no PC.</small>
  </section>;
